...
//        similar.add(computeJaccardSimilarity(queMap, dic.get(i).get(j)));
        similar.add(computeJaccardSimilarity2(queMap, dic.get(i).get(j)));
//        similar.add(computeDiceSimilarity(queMap, dic.get(i).get(j)));
//        similar.add(computeDiceSimilarity2(queMap, dic.get(i).get(j)));
      }
...
*/
  private double computeJaccardSimilarity2(Map<String, Integer> queryVector,
	    ...
	    it = docVector.entrySet().iterator();
	    aOrB = queryVector.size();
	    while (it.hasNext()) {
	      Map.Entry<String, Integer> docMap = it.next();
	      if (!queryVector.containsKey(docMap.getKey())) {
		      aOrB ++; 
	      }
	    }
